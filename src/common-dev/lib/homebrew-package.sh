...
# https://github.com/devcontainers-contrib/features/blob/main/src/homebrew-package/install.sh
# Adapted to install brew packages in bulk, faster
...
install_many_via_homebrew() {
  packages=$1
  version=latest

for package in "${packages[@]}"; do
  if [[ $(command -v "$package") ]]; then
    echo "skipping installed package $package"
    continue
  fi

  install_via_homebrew "${package}" "${version}"
